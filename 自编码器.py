...
class AdditiveGaussianNoiseAutoencoder(object):
    def _init_(self,n_input,n_hidden,transfer_function=tf.nn.softplus,optimizer=tf.train.AdamOptimizer(),scale=0.1):
        ...
        self.hidden=self.transfer(tf.add(tf.matual(
            self.x+scale*tf.random_normal((n_input,)),self.weights['w1']
            ),self.weights['b1']))
        self.reconstruction=tf.add(tf.matual(self.hidden,self.wieghts['w2']),self.weights['b2'])
    ...
    def partial_fit(self,X):
        cost,opt=self.sess.run((self.cost,self.optimizier),
...
autoencoder=AdditiveGaussianNoiseAutoencoder(n_input=784,n_hidden=200,
                                             ...
                                             optimizer=tf.train.AdamOptimizier(learning_rate=0.001),
                                             scale=0.01)
